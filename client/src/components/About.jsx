import './About.css';

const highlights = [
  { icon: '🎓', label: 'B.Tech AI (CSE)', sub: 'PIET, 2023–2027' },
  { icon: '📍', label: 'Visakhapatnam', sub: 'Andhra Pradesh' },
  { icon: '⚡', label: 'MERN Stack', sub: 'Frontend + Backend' },
  { icon: '🤖', label: 'AI/ML Explorer', sub: 'Currently learning' },
];

export default function About() {
  return (
    <section className="section about" id="about">
      <div className="container">
        <div className="about__grid">
          <div className="about__content">
            <p className="section-label">About me</p>
            <h2 className="section-title">
              A developer who loves<br />
              solving real problems.
            </h2>
            <p className="about__text">
              I'm a B.Tech Artificial Intelligence student at Parul Institute of Engineering &
              Technology, currently in my second year (2023–2027). My journey into coding started
              with curiosity about how websites work — and now I build full-stack applications that
              are fast, accessible, and actually useful.
            </p>
            <p className="about__text">
              Outside of coursework, I've independently built MERN applications from scratch —
              handling everything from database design to deployment. I'm currently expanding into
              Java, Machine Learning, and AI-powered products.
            </p>

            <div className="about__links">
              <a href="mailto:pavankumarch326@gmail.com" className="about__link">
                <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                  <path d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                </svg>
                pavankumarch326@gmail.com
              </a>
              <a href="https://github.com/PavankumarCH22" target="_blank" rel="noreferrer" className="about__link">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M12 2C6.477 2 2 6.477 2 12c0 4.42 2.865 8.17 6.839 9.49.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12c0-5.523-4.477-10-10-10z" />
                </svg>
                GitHub
              </a>
              <a href="https://linkedin.com" target="_blank" rel="noreferrer" className="about__link">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                </svg>
                LinkedIn
              </a>
            </div>
          </div>

          <div className="about__highlights">
            {highlights.map(h => (
              <div key={h.label} className="highlight-card">
                <span className="highlight-card__icon">{h.icon}</span>
                <div>
                  <p className="highlight-card__label">{h.label}</p>
                  <p className="highlight-card__sub">{h.sub}</p>
                </div>
              </div>
            ))}
          </div>
        </div>
      </div>
    </section>
  );
}
